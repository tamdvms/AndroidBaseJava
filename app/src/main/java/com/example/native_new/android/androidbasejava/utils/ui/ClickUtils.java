package com.example.native_new.android.androidbasejava.utils.ui;
...
public class ClickUtils {
    ...
    private static Drawable createStyleDrawable(Drawable src, int style, float value) {
        ...
        if (style == PRESSED_BG_ALPHA_STYLE) {
            createAlphaDrawable(pressed, value);
        } else if (style == PRESSED_BG_DARK_STYLE) {
            createDarkDrawable(pressed, value);
        }
    ...
    public abstract static class OnDebouncingClickListener implements View.OnClickListener {
        ...
        protected OnDebouncingClickListener() {
            this(true, DEBOUNCING_DEFAULT_VALUE);
        }

        protected OnDebouncingClickListener(final boolean isGlobal) {
            this(isGlobal, DEBOUNCING_DEFAULT_VALUE);
        }

        protected OnDebouncingClickListener(final long duration) {
            this(true, duration);
        }

        protected OnDebouncingClickListener(final boolean isGlobal, final long duration) {
            mIsGlobal = isGlobal;
            mDuration = duration;
        }
        ...
        public final void onClick(View v) {
            if (mIsGlobal) {
                if (mEnabled) {
                    mEnabled = false;
                    v.postDelayed(() -> mEnabled = true, mDuration);
                    onDebouncingClick(v);
                }
            } else {
    ...
    public abstract static class OnMultiClickListener implements View.OnClickListener {
        ...
        protected OnMultiClickListener(int triggerClickCount) {
            this(triggerClickCount, INTERVAL_DEFAULT_VALUE);
        }

        protected OnMultiClickListener(int triggerClickCount, long clickInterval) {
